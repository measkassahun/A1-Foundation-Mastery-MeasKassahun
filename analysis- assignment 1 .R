...
count_in_location<-function(num_setting){
  num_in_place<-sum(str_detect(locations, num_setting))
  result<-paste("There were", num_in_place,"protests in",num_setting)
  return(result)
}
...
count_on_date<-function(num_duration){
  num_in_region<-sum(str_detect(dates, num_duration))
  result<-paste("there were", num_in_region, "protest in", num_duration)
  return(result)
}
